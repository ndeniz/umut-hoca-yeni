package AstaticLocalInstanceV.q027;

public class CheckingAccount {
	public int amount;
	// line n1
	public static void main(String[] args) {
		CheckingAccount acc = new CheckingAccount();
		// line n2
		System.out.println(acc.amount);    //bunu biz 100 olup olmadini gormek icin ekeldik
	}
}
/*
 * 		Which three pieces of code, when inserted independently, set the value of amount to 100?

 	A. At line n1 insert:            +
 		public CheckingAccount(){
 		amount = 100;					
 		}
 		
 	 B. At line n2 insert:          -
 		this.amount = 100;   			// ==>Cannot use this in a static  context
 		
 	 C. At line n2 insert:          -
 		amount = 100;					// ==> Cannot make a static reference to the non-static field amount
 		
 	 D. At line n1 insert:          +
 		public CheckingAccount(){
 		this.amount = 100;
 		}
 		
 	 E. At line n2 insert:          +
 		acc.amount = 100;
 		
 	 F. At line n1 insert:
 		public CheckingAccount(){
 		acc.amount = 100;				// ==> acc cannot be resolved to a variable  obj yi main in diisnda caguramadik
 		}
 
 */
//cevap A D E

